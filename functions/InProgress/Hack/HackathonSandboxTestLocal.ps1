...
Param(
    ...
    # LocalAppxMsix
    [Parameter(HelpMessage = 'Path to a local .appx or .msix package to install in the Sandbox.')]
    [ValidateScript({
                if ($_) {
                    if (-Not (Test-Path -Path $_)) { throw "$_. does not exist" }
                    if ($_ -notmatch '(?i)\.(appx|msix|msixbundle)$') { throw "$_ is not a .appx, .msix, or .msixbundle file" }
                }
                return $true
            })]
    [String] $LocalAppxMsix,
    # Switches
...
$localAppxMsixFileName = $null
if ($LocalAppxMsix) {
    $localAppxMsixFileName = Split-Path $LocalAppxMsix -Leaf
    Copy-Item -Path $LocalAppxMsix -Destination $script:TestDataFolder -Force -ErrorAction SilentlyContinue
}
...
# Install local Appx/Msix if provided
if (Test-Path "$localAppxMsixFileName") {
    Write-Host @'
--> Installing local Appx/Msix package
'@
    `$logPath = "$localAppxMsixFileName" + ".install.log"

 
    write-host `$logPath
    try {
        Add-AppxPackage -Path "$localAppxMsixFileName" -ErrorAction Stop *>&1 | Tee-Object -FilePath `$logPath
        `$appname = (Join-Path -Path $script:SandboxTestDataFolder -ChildPath "$localAppxMsixFileName")
        Add-AppxPackage -Path "`$appname" -ErrorAction Stop *>&1 | Tee-Object -FilePath `$logPath
        write-host `$appname
        write-host `$logPath
        Write-Host "Local Appx/Msix package installed successfully. Log: `$logPath"
    } catch {
        Write-Host -ForegroundColor Red "Failed to install local Appx/Msix package. See log:`$logPath"
...
# If running locally and Launch parameter is set, launch the app
